...
export function DashboardPage() {
  ...
  return <main className="dashboard-layout">
    ...
    <section className="workspace-grid">
      <aside className="project-panel"><div className="section-heading"><div><p className="eyebrow">Navegação / {projects.length.toString().padStart(2, '0')}</p><h2>Projetos</h2></div><button className="icon-button" onClick={() => { setEditingProject(null); setShowProjectForm(true) }} aria-label="Criar projeto">+</button></div>{isLoading ? <p className="muted">Carregando projetos...</p> : projects.length === 0 ? <div className="empty-state"><p>Nenhum projeto ainda.</p><button className="text-button" onClick={() => setShowProjectForm(true)}>Criar o primeiro</button></div> : <div className="project-list">{projects.map((project) => <div key={project.id} className={`project-row ${selectedProjectId === project.id ? 'selected' : ''}`}><button className="project-select" onClick={() => setSelectedProjectId(project.id)}><span>{project.name}</span><small>{project.description || 'Sem descrição'}</small><i>abrir projeto <b>→</b></i></button><button className="project-delete" onClick={() => void removeProject(project)} aria-label={`Excluir projeto ${project.name}`} title="Excluir projeto">×</button></div>)}</div>}</aside>
